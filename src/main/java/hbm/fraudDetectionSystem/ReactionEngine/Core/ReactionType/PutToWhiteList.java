...
import hbm.fraudDetectionSystem.CommonEngine.CommServiceWhiteList.SpringLogic.FraudWhiteListServiceImpl;
...
@Slf4j
public class PutToWhiteList implements Runnable {
    ...
    protected final FraudWhiteListServiceImpl fraudWhiteListService;

    public PutToWhiteList(String reqId, FraudReaction reaction, Map<String, String> transactionData) {
        this.reqId = reqId;
        this.reaction = reaction;
        this.transactionData = transactionData;
        this.fraudWhiteListService = ApplicationContext.getBean("fraudWhiteListServiceImpl", FraudWhiteListServiceImpl.class);
    }
    ...
    public void run() {
        ...
        LOGGER.info(
                String.format(
                        "START PROCESSING REACTION: [%s] - [ATTR_WHITE_LIST]",
                        reaction.getId()
                )
        );
        ...
        try {
            ...
            WhitelistDataObject dataObject = gson.fromJson(this.reaction.getActionValue(), WhitelistDataObject.class);
            ...
            this.fraudWhiteListService.addWhiteList(
                    ...
                    "Reaction put attr to white list"
            );
            LOGGER.info("Attribute put successfully");
            LOGGER.info(
                    String.format(
                            "FINISH PROCESSING REACTION: [%s] - [ATTR_WHITE_LIST], STATUS: [SUCCESS]",
                            reaction.getId()
                    )
            );
        } catch (Exception e) {
    ...
    @Getter
    @AllArgsConstructor
    private static class WhitelistDataObject {
