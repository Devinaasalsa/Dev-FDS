...
@Slf4j
public class ConstantEngine extends DSLResolver {
    private Logger LOGGER = LoggerFactory.getLogger(getClass());
    protected final Map<String, String> transactionData;
    protected boolean evaluateStatus = false;

    public ConstantEngine(Map<String, String> transactionData) {
    ...
    public void run(String expression) throws Exception {
        ...
        String[] temp = resolvedCondition.split(" ");
        String constantValue = temp[temp.length - 1];
        String removeQuote = constantValue.replaceAll("'", "");

        if (new TypeChecker().isNum(removeQuote)) {

            LOGGER.info(
                    String.format(
                            "Constant value is integer: %s",
                            constantValue
                    )
            );

            resolvedCondition = resolvedCondition.replaceAll(constantValue, removeQuote);
